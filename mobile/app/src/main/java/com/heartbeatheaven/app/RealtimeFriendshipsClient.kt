...
class RealtimeFriendshipsClient(
    private val accessTokenProvider: () -> String,
    private val userId: String,
    private val apiKey: String,
    private val onFriendshipChange: () -> Unit
) {
    ...
    private fun connect() {
        ...
        val topic = "realtime:friendships-$userId"
        ...
        socket = client.newWebSocket(
            ...
            object : WebSocketListener() {
                override fun onOpen(webSocket: WebSocket, response: Response) {
                    ...
                    val eventTypes = listOf("INSERT", "UPDATE", "DELETE")
                    for (eventType in eventTypes) {
                        changes.put(
                            JSONObject()
                                .put("event", eventType)
                                .put("schema", "public")
                                .put("table", "friendships")
                                .put("filter", "requester_id=eq.$userId")
                        )
                        changes.put(
                            JSONObject()
                                .put("event", eventType)
                                .put("schema", "public")
                                .put("table", "friendships")
                                .put("filter", "addressee_id=eq.$userId")
                        )
                    }
                    ...
                    runCatching {
                        ...
                        when (root.optString("event")) {
                            "postgres_changes" -> {
                                ...
                                if (record != null) {
                                    val requesterId = record.optString("requester_id")
                                    val addresseeId = record.optString("addressee_id")
                            ...
                            "system" -> {
                                val message = root.optJSONObject("payload")?.optString("message").orEmpty()
                                if (message.contains("expired", true)) {
