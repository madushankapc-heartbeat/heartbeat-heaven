...
internal fun AccountScreen() {
    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        Text("Your email and mobile number stay private. Other users see your username only.", color = MaterialTheme.colorScheme.onSurfaceVariant)
        ...
        if (session != null) {
            ...
            Text("Enter the 6-digit code sent to your mobile number.", color = MaterialTheme.colorScheme.onSurfaceVariant)
            OutlinedTextField(otp, { otp = it.filter(Char::isDigit).take(6) }, label = { Text("Verification code") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            ...
            }, modifier = Modifier.fillMaxWidth()) { Text(if (busy) "Verifying..." else "Verify code") }
            ...
            OutlinedTextField(identifier, { identifier = it }, label = { Text(if (phoneMode) "Mobile number (+country code)" else "Email") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            OutlinedTextField(password, { password = it }, label = { Text("Password") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            if (phoneMode) Text("Use the full international number, for example +947XXXXXXXX.", color = MaterialTheme.colorScheme.onSurfaceVariant)
            Button(enabled = !busy && identifier.isNotBlank() && password.length >= 6 && (!signup || username.trim().length >= 3), onClick = {
                busy = true; message = null; error = null
                val creatingPhoneAccount = signup && phoneMode
                Thread {
                    val result = if (signup) api.signUp(identifier, password, username, gender, phoneMode)
                    else api.signIn(identifier, password, phoneMode).map { "Welcome, ${it.profile.username}." }
                    android.os.Handler(android.os.Looper.getMainLooper()).post {
                        busy = false
                        result.onSuccess { text ->
                            message = text
                            if (creatingPhoneAccount) phoneVerifyMode = true else session = api.currentSession()
