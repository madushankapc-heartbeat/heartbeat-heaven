    ...
    Column(Modifier.fillMaxSize().verticalScroll(rememberScrollState()).padding(20.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        if (session != null) {
            ...
            Card(Modifier.fillMaxWidth()) {
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Button(enabled = !busy, onClick = {
                        ...
                        Thread {
                            api.signOut()
                            android.os.Handler(android.os.Looper.getMainLooper()).post { session = null; busy = false; message = "You are signed out." }
                        }.start()
            ...
            Button(enabled = !busy && identifier.isNotBlank() && password.length >= 6 && (!signup || username.trim().length >= 3), onClick = {
                ...
                Thread {
                    val result = if (signup) api.signUp(identifier, password, username, gender, phoneMode).map { it }
                    else api.signIn(identifier, password, phoneMode).map { "Welcome, ${it.profile.username}." }
                    android.os.Handler(android.os.Looper.getMainLooper()).post {
                        busy = false
                        result.onSuccess { text -> message = text; session = api.currentSession() }.onFailure { error = it.message ?: "Authentication failed." }
                    }
                }.start()
