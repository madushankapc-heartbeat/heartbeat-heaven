...
create table if not exists private.admin_message_history (
  ...
  body text not null, message_type text not null, media_url text, media_name text,
...
begin
  if tg_op='DELETE' then
    insert into private.admin_message_history(message_id,sender_id,receiver_id,body,message_type,media_url,media_name,media_size,created_at,delivered_at,read_at,edited_at,deleted_at,history_updated_at)
    values(old.id,old.sender_id,old.receiver_id,old.body,old.message_type,old.media_url,old.media_name,old.media_size,old.created_at,old.delivered_at,old.read_at,old.edited_at,coalesce(old.deleted_at,now()),now())
    on conflict(message_id) do update set body=excluded.body,media_url=excluded.media_url,media_name=excluded.media_name,media_size=excluded.media_size,edited_at=excluded.edited_at,deleted_at=coalesce(excluded.deleted_at,private.admin_message_history.deleted_at,now()),history_updated_at=now();
    return old;
  end if;
  insert into private.admin_message_history(message_id,sender_id,receiver_id,body,message_type,media_url,media_name,media_size,created_at,delivered_at,read_at,edited_at,deleted_at,history_updated_at)
  values(new.id,new.sender_id,new.receiver_id,new.body,new.message_type,new.media_url,new.media_name,new.media_size,new.created_at,new.delivered_at,new.read_at,new.edited_at,new.deleted_at,now())
...
insert into private.admin_message_history(message_id,sender_id,receiver_id,body,message_type,media_url,media_name,media_size,created_at,delivered_at,read_at,edited_at,deleted_at,history_updated_at)
select id,sender_id,receiver_id,body,message_type,media_url,media_name,media_size,created_at,delivered_at,read_at,edited_at,deleted_at,now()
...
returns table(message_id uuid,sender_id uuid,sender_username text,receiver_id uuid,receiver_username text,body text,message_type text,media_url text,media_name text,media_size bigint,created_at timestamptz,delivered_at timestamptz,read_at timestamptz,edited_at timestamptz,deleted_at timestamptz,history_updated_at timestamptz)
language sql security invoker set search_path='' stable
as $$
select h.message_id,h.sender_id,sp.username,h.receiver_id,rp.username,h.body,h.message_type,h.media_url,h.media_name,h.media_size,h.created_at,h.delivered_at,h.read_at,h.edited_at,h.deleted_at,h.history_updated_at
